...
from ..text_cleaner import PlateTextCleaner
...
class PlateOCR(BaseOCR):
    ...
    def __init__(self):
        ...
        self.text_cleaner = PlateTextCleaner()
    ...
    def extract_text(self, image: np.ndarray) -> Tuple[Optional[str], float]:
        ...
        try:
            # Preprocessing
            processed_variants = self.preprocessor.plate_preprocess(image)
            ...
            for variant_idx, processed_img in enumerate(processed_variants[:4]):
                ...
                try:
                    ...
                    if paddle_results and paddle_results[0]:
                        ...
                        if text:
                            cleaned = self.text_cleaner.plate_clean_text(text)
                            if cleaned:
            ...
            if all_results:
                ...
                print(f"[SUCCESS] Best plate: {best_text} (score: {best_score:.3f}, method: {best_method})")
                return best_text, best_score
            
            return None, 0.0
            
        except Exception as e:
            print(f"[ERROR] Plate OCR failed: {e}")
            return None, 0.0
